...
- [Five a Day Evolution](#five-a-day-evolution)
  ...
  - [Testing Environment (QA)](#testing-environment-qa)
    ...
    - [For developers: how the QA environment works](#for-developers-how-the-qa-environment-works)
      - [Access control for `/app/testing/`](#access-control-for-apptesting)
...
- v1.30.0 mounted the management app under `settings.APP_URL_PREFIX` and gave `/` to the public React site. Two hand-written paths did not move with it: the QA dashboard's backlog buttons posted to `/api/testing/backlog/<id>/update/` and the Desarrollos board linked rows to `/testing/features/<id>/`. Both 404.
- The symptom is what made this expensive to diagnose: a 404 body is **HTML**, and `apiFetch` can only report an unparseable body as the generic *"El servidor ha devuelto un error"*. Two buttons therefore blamed the server for a release and a half, and every investigation started in the wrong place. The Desarrollos links failed the same way, one click later.
- Both now come from `{% url %}` with a `0` placeholder that JS swaps the id into — the same pattern `expenses.html` already used, so Django owns the whole path including the prefix.
- **A guard replaces the two fixes**: `test_no_hand_written_url_misses_the_app_mount_prefix` reads every template and JS module and fails on any quoted `/api/…`, `/students/…` or `/testing/…` literal that is neither `{% url %}`-derived nor prefixed with `${window.APP_PREFIX}`. It found all three offenders before they were fixed. It skips comments, because several of them legitimately quote an example path while explaining the rule.

**The public contact form could not submit at all outside development**

- `CSRF_COOKIE_HTTPONLY` is `True` whenever `DEBUG` is off, so on the testing VM and in production JavaScript cannot read the `csrftoken` cookie. The React form read it anyway, sent an empty `X-CSRFToken`, and every submission was refused with a **403** whose HTML body the handler cannot parse — so it fell through to "no hemos podido enviar el mensaje", which reads as a mail outage and sends the investigation to the SMTP layer.
- It worked in development for the one reason that lets this class of bug reach production: with `DEBUG=True` the cookie is not HttpOnly.
- The fix keeps the app's existing rule rather than inventing a second one. Every other page takes the token from markup Django rendered — the hidden input in `base.html`, read by `base.js`. A built file has no `{% csrf_token %}` tag, so `frontend_index` now injects the equivalent: a **`<meta name="csrf-token">`** tag, published per request into the shell it already serves.
- The alternatives were worse and are recorded as refused: disabling `CSRF_COOKIE_HTTPONLY` weakens every page in the app to fix one form, and `@csrf_exempt` would be the fourth such view where the three that exist each justify it the same way — the caller is a **server** carrying its own credential, which a public browser POST cannot claim.

**Reply on a contact enquiry answered the academy itself**

- The visitor's address was passed as template *context* only, so it rendered as a `mailto:` button in the body while the message carried no `Reply-To` header. `From:` is the academy's own SMTP account and cannot be anything else, so pressing Reply answered the academy's own inbox — the academy writing to itself — and the family never heard back.
- `EmailService.send_email` gained `reply_to` in v1.30.4 for exactly this, and `submit_portfolio_contact` uses it. The academy's own form, which runs every day, did not.

**The acknowledgement is a dialog now**

- The success state used to **replace** the form, so a family sending a correction or asking about a second child had no way back except reloading. [`ContactSuccessDialog`](frontend/src/components/ContactSuccessDialog.jsx) sits on top and the form is cleared underneath it.
- It is a real dialog, not styled text: `role="dialog"` + `aria-modal`, an accessible name, focus moved in on open and restored to the trigger on close, Escape, backdrop click, a tab loop and a body scroll lock. A modal that only *looks* modal tells a screen reader nothing happened and leaves a keyboard user tabbing through a form they can no longer see.
- **Clearing it with `form.reset()` was wrong and fails invisibly.** React keeps its own tracker of each input's last value; a native reset changes the DOM without going through React, so the tracker still holds the old text and the next keystroke replays it — typing "Ana" into a visibly empty box yields "AnaAna", and the doubled email then fails HTML validation so the next message is never sent at all. The fields are **controlled** instead, which also revives `formData`: state the component wrote on every keystroke and never read.
- Three floating layers were all `z-50` (navbar, WhatsApp popup, dialog), so DOM order decided and the popup — rendered last by `Layout` — painted over the dialog, landing squarely on its buttons on a phone. The dialog is `z-[60]`. Only a screenshot showed it.

**A validation policy for addresses typed by the public**

- [`core/email_policy.py`](project/core/email_policy.py) is a leaf module that answers with an error **code**, so the wording stays in the view — the academy's site is Spanish and the portfolio relay's is English, and both can reuse the rule.
- Deliberately **stricter than Django's `EmailValidator`**, which accepts forms no family types and each of which is a hazard downstream: a quoted local part can hide an `@` or a comma (`"ana@evil.test"@gmail.com`), an address literal (`ana@[192.168.0.1]`) has no domain to check at all, and a non-ASCII address is legal but cannot be relied on to route through this academy's Gmail — taking an enquiry we then fail to answer is worse than refusing it plainly. Tests assert *both* halves, so they fail if a future Django tightens up and the justification quietly stops being true.
- RFC length caps are checked **first**, so the work is bounded on hostile input; whitespace is refused anywhere in the address, because a newline is the shape header injection takes.
- **A 40-domain allowlist** covering the global consumer providers with their `.es` spellings, the privacy providers, and the Spanish ISPs where long-standing family addresses still live. Membership, not suffix matching — `endswith` would accept `gmail.com.evil.test`.
- **The cost is stated rather than hidden**: an allowlist refuses real people as a side effect of refusing throwaway addresses. So the refusal names another provider, WhatsApp *and* the academy's address; it is a different message from a syntax error (telling somebody who mistyped that their *provider* is unacceptable sends them off to create an account they did not need); and each refusal logs the **domain only**, never the address, which is both the privacy-safe half and the actionable one — those lines are the evidence for what to add to the list.

**A cooldown that does not punish a typo**

- `core.rate_limit` gained `cooldown_active` / `begin_cooldown`, and the contact form starts a 60-second wait **after a message is actually sent**.
- It is deliberately not a second `@rate_limit` decorator, and that distinction is the whole point: a decorator claims its slot **before** the view runs, so a submission the view then refuses would still spend it. The commonest refusal here is a mistyped email — the family would be told to correct the address and then refused for a minute when they did, the site appearing to break at the exact moment they fixed their own mistake. The existing 5-per-10-minutes window still counts every attempt, which is what bounds abuse.
- The cooldown answers **JSON**, unlike the limiter's own `text/plain` 429, so the form shows the academy's wording instead of blaming the send for a wait. The button counts the same number down, and a test pins the two copies of that number against drift.
- The helpers honour `RATELIMIT_ENABLE` like the decorator: the cache is shared across the suite and every test client is `127.0.0.1`, so a cooldown left set would throttle an unrelated test. The tests that exercise it switch it on explicitly — without that they would pass vacuously, proving only that the control is off.

**Testing**

- New [`test_email_policy.py`](project/tests/unit/test_email_policy.py) covering every refusal branch, the Django-parity assertions, and a property check that every domain on the allowlist is actually reachable by the validator (a typo in the list is invisible: the entry simply never matches).
- [`test_frontend_site.py`](project/tests/integration/test_frontend_site.py) gained the CSRF-meta end-to-end assertion under `enforce_csrf_checks`, the `Reply-To` pin, the refuse-then-correct cooldown sequence and the cooldown drift check; [`test_frontend_invariants.py`](project/tests/integration/test_frontend_invariants.py) gained the app-prefix guard.
- The existing contact tests submitted `ana@example.com`, which the allowlist correctly refuses — those fixtures now use a real provider.

</details>

<details id="v1310">
<summary><strong>v1.31.0 — Public tester role, per-page SEO for the public site</strong></summary>
...
- `five-a-day-frontend` (Vite + React 19 + react-router 7 + Tailwind 4) is **migrated into this
  repo**. Sources live in `frontend/`; `package.json`, `vite.config.js` and `eslint.config.js`
  sit at the repo root beside `pyproject.toml` — one place per repo for "how is this built",
  whichever half of the stack you mean.
- **The management app moved from `/` to `/app/`.** `settings.APP_URL_PREFIX` is the single
  owner of that prefix: the root URLconf, `SimpleAuthMiddleware`, `@admin_required`, the PWA
  manifest and `window.APP_PREFIX` all derive from it rather than re-typing `/app`. `/admin/`
  moved with it (`/app/admin/`); `/health/`, `/static/` and `/media/` deliberately stayed at the
  root — the deploy pipeline, the QA sign-off gate and the uptime checks all poll `/health/`.
- Django serves the built SPA: `core.views.frontend` returns `frontend/dist/index.html` for `/`
  and for each route in `SPA_ROUTES`, and WhiteNoise serves everything beside it from the origin
  root (`WHITENOISE_ROOT`), so `/images/…` and `/videos/…` resolve at the absolute paths the
  React sources already use. Routes are **enumerated, not a catch-all** — React Router has no
  catch-all either, so a catch-all would answer every typo with a 200 and a blank page.
- `SimpleAuthMiddleware` now returns early for any path outside the prefix. Without it, visiting
  `/` while logged out redirected to the staff login — the one thing a marketing site must never
  do.

**Contact form**

- The site's form was a **Netlify Form**, which stops existing the moment Django serves the site.
  It now posts to `/api/contact/` and emails the academy (`CONTACT_FORM_RECIPIENT`, defaulting to
  `DEFAULT_FROM_EMAIL`), rate-limited, honeypotted and length-capped.
- The React handler used to set "mensaje enviado" inside its `try` **without reading the
  response**, so a form delivering nowhere looked identical to one that worked. It now checks the
  status; twelve Vitest cases cover the failure paths.

**Responsive pass**

- Audited across 15 viewports in real Chromium (`tools/responsive_audit.py`). Laptops and big
  screens are unchanged — every fix is gated at `lg`.
- `/nuestra-metodologia` scrolled sideways by 16px on **every** viewport below 1024px; the card
  overhang is now `lg`-only.
- The public header shrinks below `lg` (172px → 116px on a 375px phone), the hamburger went
  28×28 → 44×44, and both carousels' arrows and dots got real hit areas.
- **The app sidebar is an off-canvas drawer below `lg`** — it was a fixed 7.5rem at every width,
  38% of a 320px phone. Content width on that phone went 200px → 320px. `lg:transform-none`,
  not `lg:translate-x-0`: a transform makes the sidebar the containing block for its
  `position: fixed` children and would move the desktop icon rail.
- The purple contact bar keeps a **14px floor** and wraps to more rows rather than shrinking —
  these are a phone number and an email somebody is meant to read.
...
- New `make frontend-build / -test / -dev / -lint`, a pinned Node stage in the `Dockerfile`, a
  `Frontend` CI job gating `docker-publish`, and `frontend-lint` + `frontend-test` pre-commit
  hooks.

**Fixes found by the new tests**

- `data.js` pointed "Saber más" at `/los-ods`; the route is `/ods` — a dead link on the live site.
- `QuienesSomos` had no `<h1>` (the only page without one) — added `sr-only`.
- The contact form's labels had no `htmlFor`/`id`, so screen readers announced unnamed controls.
- `npm audit`: 5 high-severity advisories in **production** dependencies, now 0.

</details>

<details id="v12912">
<summary><strong>v1.29.12 — A blank card, a silent deploy failure, and a consent screen that cannot be published</strong></summary>
...
- The academy's first day of real data began with a database still holding the pre-launch test
  families. Deleting them from `/admin/` was impossible for an admin, a superuser, and the Google
  OAuth account alike — Django answered *«La eliminación del Pagos seleccionado resultaría en el
  borrado de objetos relacionados, pero su cuenta no tiene permisos para borrar los siguientes
  tipos de objetos: Pago»*.
- Nothing was misconfigured. Two deliberate protections were talking through Django's one generic
  message: `PaymentAdmin.has_delete_permission` refuses a collected, refunded or receipt-numbered
  row (v1.29.2, so a fiscal record is never one click from a hole in the `YYYY-NNN` sequence), and
  `Student` / `Parent` / `Enrollment` are `PROTECT`ed by the rows that bill them. The confirmation
  page aborts on the first protected object, so a selection containing one cobrado payment blocked
  every other row with it.
- The result read as a broken account rather than a rule, and left the only route to a legitimate
  deletion being a database shell — which is exactly how production's test data was finally purged
  (a one-off `gcloud run jobs execute ... --args` override against the migrate job).

**`purge_with_history` — a second action, named for what it does**

- `core/admin_purge.py` adds `PurgeWithHistoryMixin`: it deletes the protecting rows in FK order
  and then the selection, behind a confirmation page that names every model and count it is about
  to destroy, and reports the exact per-model totals afterwards.
- Wired into four admins, each declaring only the reverse accessors that protect it: **Alumnos**
  (`payments` → `enrollments`), **Familiares** (`payments`), **Matrículas** (`payments`) and
  **Pagos** (nothing — the action exists there purely to get past the fiscal guard).
- The ordinary Delete button **keeps** its guard. The protection was never meant to stop a
  deliberate, named decision, only an accidental click; a receipt-numbered payment still survives
  `delete_selected` and yields only to the action a human chose by name.
- Superuser-only, and not even listed otherwise — `Teacher.admin` mirrors onto `is_superuser`, so
  in practice that means any admin teacher.

**Why the module imports nothing but Django**

- Both `billing.admin` and `students.admin` use it, and naming `Payment` in a shared helper would
  invert the documented `students <- billing` dependency for one of them. Each ModelAdmin declares
  the reverse **accessor names** instead, which `_meta` resolves to a queryset with no import at
  all — the same leaf-module discipline as `billing/money.py` and `core/date_utils.py`.
- The action re-selects its targets by pk before deleting: a ModelAdmin queryset carries
  `select_related` and, on `EnrollmentAdmin`, aggregate annotations — a `GROUP BY` that has no
  business being in a `DELETE`.

**Testing**

- `TestRecordsCanBeRemovedDeliberately` (9 cases) drives the real admin URLs and template: the
  confirmation page deletes nothing, confirming removes a student with their payments, enrollments,
  `StudentParent` links and Fun Friday rows while leaving the guardian standing, a receipt-numbered
  payment survives `delete_selected` and yields to the purge, and a non-superuser is never offered
  the action on any of the four admins — nor served by it when called directly.
- Hiding the action goes through `@admin.action(permissions=["purge"])` + `has_purge_permission`
  rather than a `get_actions` override. The override worked, but on Django 6.1 it sat on two APIs
  that go in 7.0 (the `action_location` signature, and unpacking an `Action` as a tuple), and the
  suite's only remaining deprecation warnings are the email layer's — a new class of them would
  have hidden inside that noise until the Django 7 bump.

**The second half: logs that could not answer «what failed, and why?»**

- Three separate defects, each of which alone made an incident hard to read and which together
  made one unreadable. A production welcome-email failure arrived as **three alert mails**, one
  incident, and no way to prove they were the same event.
- **Severity was a lie.** `logging.StreamHandler` defaults to `sys.stderr`, and Cloud Run labels
  everything a container writes to stderr as ERROR — so every `logger.info` line arrived in Cloud
  Logging indistinguishable from a real failure, and the console's severity filter, the first
  control anyone reaches for mid-incident, selected the entire log. The console handler moves to
  **stdout** and, in production, through `CloudLoggingFormatter`: one JSON object per line
  carrying an explicit `severity`, `sourceLocation`, and a trace field. `LOG_FORMAT=json|text`
  overrides; the default is text everywhere a person reads the logs with `docker compose logs`.
- **Nothing tied one incident's records together.** Every record now carries a `request_id`.
  `RequestLogMiddleware` binds it (FIRST in `MIDDLEWARE`, so even `SecurityMiddleware`'s SSL
  redirect is joinable), `RequestContextFilter` stamps it, it is echoed as the `X-Request-ID`
  response header so a user reporting a problem can quote the exact request, and it appears in
  both the `mail_admins` alert and the QA error mail. Celery **inherits** the id when a task was
  queued by a request — production runs eager, so it usually was — and falls back to the task id
  otherwise; `task_failure` now logs one ERROR naming the task and its cause, never its
  arguments.
- **Volume.** One levelled line per request: 5xx → ERROR, slow → WARNING past
  `SLOW_REQUEST_LOG_MS` (3 s), 4xx → INFO, healthy → DEBUG, i.e. nothing in production. A 5xx
  Django *already* logged drops to INFO, because `django.request` logs the traceback at ERROR and
  that is what mails the admins — a second ERROR would mean two alerts per incident out of two
  throttle buckets. It logs the resolved **URL name**, never `request.path`, which is
  attacker-controlled free text and groups badly. And `django.utils.autoreload` is pinned to
  INFO: it emits one DEBUG line per watched file, which under the dev default was **592 of the
  last 600 lines** in a container.

**Silent failures that now say something**

- `EmailService.send_email` no longer passes `fail_silently` down to Django. It was policy
  belonging to *this* method, and handing it to the backend buried the cause inside Django
  (`open()` swallows `OSError`, `_send()` swallows `SMTPException`) leaving a bare zero with no
  traceback. A zero-accepted send now reports recipient counts and whether a shared connection
  was in play, and a mass send whose failures were previously a scrolling INFO tally —
  «130 fallidos» and nothing else — logs at **ERROR** so the batch itself surfaces and alerts.
  It also retires the `RemovedInDjango70Warning` on that argument.
- Four `except: pass` blocks that discarded the reason: the QA task-done notification, and two
  preview paths in `enrollment_form` where an unresolvable student id silently became
  «Alumno Ejemplo» with no way for the operator to tell. Two of them were also over-broad
  `except Exception` catches, now narrowed to what they actually expect.
- A handful of f-string log calls became lazy `%`-style with coerced ids, the shape the rest of
  the codebase uses to keep request-derived values out of a log record.

**Testing**

- `test_logging_utils.py` (44 cases) covers the formatters, the filter and the sanitizers;
  `test_middleware.py` grows from 10 to 24, driving `RequestLogMiddleware` through the real
  stack — the header precedence, the sanitizing, the `finally` that stops a reused Gunicorn
  thread leaking an id into the next request, and the whole level ladder.

</details>

<details id="v1296">
<summary><strong>v1.29.6 — A release that drops schema says so, everywhere it matters</strong></summary>
...
| Make | 55 targets, 51 of them listed by `make help` (`test-cov-gate`, `check-deploy` and `version` are deliberately unadvertised) |
...
# Line format. json writes one Cloud Logging object per line with an explicit
# severity field; text is the readable line you want behind `docker compose logs`.
# LOG_FORMAT=text                 # json in production, text elsewhere
# A request at least this slow is logged at WARNING by RequestLogMiddleware.
# SLOW_REQUEST_LOG_MS=3000
# Production only: lets Cloud Logging nest our records under the Cloud Run
# request log. Cloud Run does not set it for you. Unset just means no nesting.
...
| `TEACHER_SEED_<N>_PASSWORD` | Initial password. Omit to force activation via `/app/password-reset/`. Re-running seed never overwrites a password an admin later changed. | No | unusable password |
...
| `GOOGLE_DRIVE_RECEIPTS_FOLDER_ID` | Base "Five a Day" Drive folder id for the receipt **archive** (v1.29.0). Needs a Google account connected from `/app/management/` as well (v1.29.10) — a service account cannot write to a consumer Drive; unset disables the upload | No | — (upload off) |
...
| `GITHUB_DISPATCH_TOKEN` | Fine-grained PAT (contents: read/write) letting the `/app/testing/` sign-off button fire the `qa-ready-for-prod` `repository_dispatch` that arms `Deploy production`. Read on the testing VM only; unset means no request is made and the nightly cadence arms the deploy instead | No | — (fail-soft) |
...
| `LOG_FORMAT` | `json` or `text` (v1.29.7). JSON emits one Cloud Logging object per line carrying an explicit `severity`, so a record's level stops being inferred from the stream it arrived on; `text` is the human-readable line the QA VM and dev are read with | No | `json` in production, `text` elsewhere |
| `SLOW_REQUEST_LOG_MS` | A request taking at least this long is logged at WARNING by `RequestLogMiddleware` (v1.29.7). A slow-but-successful request appears in no other log we keep | No | `3000` |
| `GOOGLE_CLOUD_PROJECT` | Project id used to build the fully-qualified `projects/<id>/traces/<hex>` field that makes Cloud Logging nest an app record under its Cloud Run request log. Cloud Run does **not** set it for you; unset simply means no nesting, never a fabricated trace | No | — (entries not nested) |
...
│   │   ├── rate_limit.py         Cache-backed IP rate limiter (v1.10) + cooldown_active /
│   │   │                         begin_cooldown (v1.31.1 — a wait AFTER a successful
│   │   │                         action; a decorator would spend its slot on a
│   │   │                         submission the view then refuses)
│   │   ├── email_policy.py       v1.31.1 — validation for an address typed by the PUBLIC:
│   │   │                         stricter than EmailValidator, plus a 40-domain
│   │   │                         allowlist. Leaf module, returns an error CODE so the
...
│   │   ├── middleware.py         RequestLogMiddleware (v1.29.7 — FIRST in MIDDLEWARE:
│   │   │                         binds the correlation id, logs one levelled line per
│   │   │                         request, echoes X-Request-ID) + SimpleAuthMiddleware
│   │   │                         + QAErrorEmailMiddleware + NoHtmlCacheMiddleware
│   │   │                         + SecurityHeadersMiddleware (CSP + Permissions-Policy,
│   │   │                         v1.23.0)
│   │   ├── logging_utils.py      v1.29.7 — CloudLoggingFormatter (JSON + severity +
│   │   │                         sourceLocation + trace), HumanFormatter, the
│   │   │                         RequestContextFilter that stamps request_id on every
│   │   │                         record, and the header sanitizers. Stdlib-only: it is
│   │   │                         instantiated by dictConfig before the app registry exists
...
│   │   │                         (v1.29.7: purge_with_history on Student + Parent)
...
│   │   │                         (v1.29.7: purge_with_history on Payment + Enrollment)
...
│
├── frontend/                     PUBLIC SITE — Vite + React 19 + react-router 7 +
...
├── tools/                        static-analysis.env (pre-commit) +
│                                 responsive_audit.py (Playwright viewport audit)
...
| **Admin** | PaymentAdmin (status bulk actions that refuse an illegal transition, CSV export, and the v1.29.2 delete guard keeping a cobrado / reembolsado / receipt-numbered row out of reach of the Delete button), EnrollmentAdmin (annotated payment totals), ExpenseAdmin. v1.29.7 adds `purge_with_history` to the first two — the only way past that guard, by name and behind a confirmation page |
...
- **Development** — credentials checked against `LOGIN_USERNAME` / `LOGIN_PASSWORD` from the active `.env`. A matching Django superuser is get-or-created so `/app/admin/` works in the same session.
...
Public flow at `/app/password-reset/...` that lets a teacher recover access without admin intervention.

- **Request page** (`/app/password-reset/`) — enter email; if it matches a `auth.User`, an HTML email is sent (template at `core/templates/emails/password_reset.html`) with a signed reset link.
- **Sent confirmation** (`/app/password-reset/sent/`) — generic confirmation that does not disclose whether the email existed.
- **Confirm form** (`/app/password-reset/confirm/<uidb64>/<token>/`) — new-password form using Django's signed token machinery; rejects expired or replayed links.
- **Complete page** (`/app/password-reset/complete/`) — confirms the change and links back to login.
...
`/app/parent/` (v1.9, re-authenticated in v1.27) — a separate, family-facing surface with its own session, styled and themed like the rest of the app.

> **Switched OFF since v1.29.4.** `PARENT_PORTAL_ENABLED` defaults to `False`, and while it is off every `/app/parent/` URL is a 404 — login and recovery included — and no invitation or recovery email is sent. The feature below is intact and fully tested; only the switch stands in front of it. `Parent.portal_invite_sent_at` is deliberately left unstamped while it is off, so setting the flag to `True` still invites every family exactly once.

- **Email + password login** — `/app/parent/login/` is the same shape as the staff login. The password is a Django hash on `Parent`, **deliberately not an `auth.User`**: `_authenticate_teacher` authenticates any `auth.User`, so a family holding one would hold a staff login into the admin app. The portal keeps a `parent_id` session and never touches `django.contrib.auth`.
...
| [`unit/test_email_policy.py`](project/tests/unit/test_email_policy.py) | 56 | `core.email_policy`, the only thing between a public form and the academy's inbox. Every refusal branch (blank, malformed, whitespace inside the address, non-ASCII, over-length, domain off the allowlist), and the length cap asserted to run BEFORE anything scans the string. `TestStricterThanDjango` asserts BOTH halves of each extra rule — that `EmailValidator` accepts a quoted local part hiding an `@`, an address literal and an unknown domain, and that this module does not — so it fails if a future Django tightens up and the justification quietly stops being true. One property check walks the whole allowlist and asserts every entry is actually reachable by the validator, because a typo there is invisible: the entry simply never matches and the family it was added for keeps being refused |
...
| [`integration/test_middleware.py`](project/tests/integration/test_middleware.py) | 30 | `SimpleAuthMiddleware`: public paths (login, health, static, media, OAuth prefix), protected paths redirect to login, authenticated requests pass; `NoHtmlCacheMiddleware` marks dynamic HTML `no-cache` while leaving hashed static assets immutable. v1.29.7 adds `RequestLogMiddleware` through the real stack: every response carries `X-Request-ID`, an inbound `X-Cloud-Trace-Context` wins over an inbound `X-Request-ID` and both are sanitized rather than echoed (a newline in the header would forge log records), the id is reset in a `finally` so a Gunicorn thread cannot leak it into the next request it serves, and the level ladder holds — a 5xx Django already logged drops to INFO so one incident cannot raise two alerts from two throttle buckets, a slow request warns, and the resolved URL name is logged where `request.path` never is |
...
| [`integration/test_frontend_invariants.py`](project/tests/integration/test_frontend_invariants.py) | 32 | The server-observable half of the frontend pass: write controls gated on `is_admin_user`, the money on the student ficha hidden from a non-admin teacher, `<html lang="es">`, no misleading required asterisk on optional fields, and the `{{ x }} euros` / bare-number contract on the payment-reminder template. Two structural guards live here because nothing else can catch them: one asserts `tailwind-config.js` and `palette.css` declare the same palette (the inline copies had already drifted — `verify.html`'s was missing `fontFamily`), and one scans every inline `style=""` attribute by perceived lightness and fails on a light background hex outside `theme.css`'s matched set. That second test found four pre-existing dark-mode bugs on its first run. v1.30.4 makes the `lang` check allowlist-aware: `NON_SPANISH_TEMPLATES` carries the handful that are legitimately another language (the portfolio relay's email), checked against their **declared** language rather than skipped, with a companion test failing on a stale path. v1.31.1 adds the app-mount-prefix guard: it reads every template and JS module and fails on a quoted `/api/…` or `/testing/…` literal that is neither `{% url %}`-derived nor `${window.APP_PREFIX}`-prefixed — the shape that 404'd the QA backlog buttons for a release and a half, reported only as a generic server error. It blanks comments first, since several legitimately quote an example path while explaining the rule |
| [`integration/test_frontend_site.py`](project/tests/integration/test_frontend_site.py) | 65 | Django's half of serving the public React site. Parses `App.jsx` and fails if its routes disagree with `SPA_ROUTES` — drift there is silent in the direction that matters, because a route added in React but missing here works on every in-app click and 404s only on a refresh. Also: every public route returns the same shell, the shell is `no-cache` and sets a CSRF cookie (without which the contact form cannot POST at all), an unknown path is a real 404, and the contact endpoint's validation, honeypot, field-label whitelist and HTML escaping. One test asserts the contact email declares itself light-only and carries no `prefers-color-scheme`. v1.31.1 adds the three controls the form now depends on: the `<meta name="csrf-token">` Django injects into the shell, asserted end to end under `enforce_csrf_checks` because the cookie is HttpOnly outside development and a cookie reader 403s every submission; the `Reply-To` header, without which pressing Reply answers the academy's own inbox; and the cooldown — including the sequence that matters, a refused address followed by the corrected one, which must go through |
...
| `core/email_policy.py` | 44 | 2 | 95% | 163-164 |
...
|-----------|------|-------------|
| Login view (dev) | `core/views/auth.py` | When `DJANGO_ENV=development`, compares username/password against `LOGIN_USERNAME`/`LOGIN_PASSWORD` env vars and get-or-creates a matching Django superuser so `/app/admin/` works. No hardcoded fallbacks — if env vars are missing, login is refused. |
...
| Rate limiting | `core/rate_limit.py` | v1.10. Cache-backed IP throttle (`cache.add` + `cache.incr` — atomic on Redis and memcached, closes the TOCTOU race a plain `get→set` would open). Applied to `/app/login/` (5/min/IP), `/app/parent/login/` (5/min/IP), `/app/parent/login/<token>/` (20/min/IP against brute force), and `/app/two-factor/verify/` (6/min/IP). `RATELIMIT_ENABLE=False` bypasses in tests. |
...
- **Testing Tools** (the blue "info" icon at the bottom of the sidebar) — This is your QA control panel:
  ...
  - **Desarrollos** (the book card, v1.21.0) — the epic board at `/app/testing/features/`. A *desarrollo*
...
| `DJANGO_ENV` | `testing` | Like production (collectstatic, Gunicorn, secure cookies) but enables the `/app/testing/` dashboard |
...
| Password reset | `/app/password-reset/...` (public, branded templates) | Lets QA teachers without an initial password activate via email |
| Admin panel | `/app/admin/` — same Teacher session (admin teachers only) | Django admin for inspecting raw data; non-admin teachers don't see it |
...
| `/app/testing/` | In-app QA dashboard with project info, seeding, backlog and the error-reporting toggle |
| `core/decorators.py` | `qa_access_required` decorator — reusable access gate for QA-only views |

#### Access control for `/app/testing/`
...
Feature branches off `development` are welcome for non-trivial work, but the expected flow is: work on `development` → wait 3 h → auto-promoted to `testing` → **auto-deployed to the testing VM overnight** → QA signs off on `/app/testing/` → manual merge to `main` → production deploy **arms itself the moment the last of {sign-off, merge} lands and waits for your approval** (the nightly testing run remains the strict watchdog and fallback). See [DEPLOYMENT.md → CI/CD](DEPLOYMENT.md#4-cicd--automated-deploys).

### Workflows

| Workflow | File | Triggers | Purpose |
|----------|------|----------|---------|
| **CI** | [`ci.yml`](.github/workflows/ci.yml) | Push to `development`/`testing`/`main`; PRs to `development`/`testing`/`main` | Seven jobs — **Lint** (Ruff + Bandit + pip-audit + Hadolint), **Type check** (mypy), **Tests** (pytest + PostgreSQL 16 + coverage artifact), **Docker build** (validates Dockerfile), **Trivy** (filesystem CVE scan → Security tab), **Docker publish** (GHCR push + image scan, on `main`/`testing` only) and, since v1.30.0, **Frontend** (Node 22 - `npm ci`, ESLint, Vitest, Vite build, and `npm audit --omit=dev --audit-level=high` gating only what SHIPS). That job is separate from **Tests** because it needs Node and nothing else - no database, no uv, no Django - and because **Tests** can only prove Django served the SPA shell, which is the same shell it serves for every public route; whether React then renders or throws on mount is invisible to it. `Docker publish` gates on it, since the published image contains the built site. Since v1.27.1 it carries a least-privilege top-level `permissions` block (it was the only workflow without one, so jobs running arbitrary branch code had the repo-default token scope), every job has a `timeout-minutes`, and the **image** scan now GATES on HIGH/CRITICAL: a second Trivy pass writes JSON and a separate step fails the job, so a missing or unparseable report reads as a failed scan rather than a clean one. The escape hatch is [`.github/trivyignore`](.github/trivyignore), applied to the gate only so an ignored CVE still appears in the Security tab |
...
- **Locks the fresh version for production**: runs `manage.py set_ready_for_prod off` in the container and asserts `/health/?deep=1` reads `ready_for_prod: false`. Only the **¿Listo para desplegar?** button on `/app/testing/` unlocks it — that click is QA's sign-off, and it always refers to the exact version it was given on
