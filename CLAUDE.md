...
- **Two themes: light = PINK, dark = VIOLET** — the Tailwind `primary` scale in `base.html` is the original **rose/pink** (primary-500 `#f93a76`) and is the light default. The **dark** theme re-skins `primary` back to **violet** (the classic look) via `core/static/css/theme.css`, which fully overrides every `primary` utility under `html.dark` so no pink leaks into dark mode. Dark mode is a `html.dark` override sheet, NOT `dark:` variants (the app uses hard-coded utility classes). `theme.css` overrides `html.dark .<utility>` (higher specificity, no `!important`); `darkMode: 'class'` is set. The class is toggled by `core/static/js/theme.js` and applied before first paint by an inline script in `base.html`'s `<head>`; the choice persists in `localStorage` (`theme` key), default light. When adding a new `primary-*` or surface/text utility to a template, add its dark override to `theme.css` so dark mode stays violet.
