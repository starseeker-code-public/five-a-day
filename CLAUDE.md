...
- **One theme (violet) for both light and dark — for now** — the app uses the original violet `primary` palette (primary-500 `#8b5cf6`) in `base.html`, and it is the SAME for light and dark. A light/dark toggle exists in the header (`theme.js` flips `html.dark`, persisted in `localStorage` `theme` key, applied pre-paint by an inline script in `<head>`, `darkMode: 'class'`), but `core/static/css/theme.css` currently only swaps the toggle icon — `html.dark` makes NO visual change. A real dark theme will be added later as `html.dark .<utility>` overrides in `theme.css` (NOT `dark:` variants — the app uses hard-coded utility classes). Don't reintroduce a second palette without being asked. Note: a few spots hard-code the violet hex (`home.html` dashboard gradient, `403/405.html`) — changing the palette means updating those too.
