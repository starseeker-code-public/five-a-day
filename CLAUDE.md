...
- **Never return `str(e)` to the client; never log a request value raw** (v1.14.4 cleared 46 CodeQL alerts of exactly these two shapes). **Catch-alls:** `except Exception` in a view must `logger.exception(...)` and return a *fixed* message — echoing exception text leaks internals (on an `IntegrityError`, the table and column names). Genuine validation errors (`ValidationError`, `InvalidOperation`) are the exception: their message is written for the user, so keep it. **Logs:** wrap anything request-derived (headers, query string, form body, URL captures) in `safe_log()` from `core/log_safe.py`, which strips CR/LF/ESC so it can't forge log records. `comms` uses its own `_safe_log` twin rather than importing from `core`. Two known-intentional alerts carry `# codeql[...]` suppressions with rationale: the settings star-import in `settings_test.py` and the `print()` in `scripts/generate_secure_password.py`.
