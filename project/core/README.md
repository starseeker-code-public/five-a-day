...
| ------ | ----- | ----------- |
| `auth.py` | `login_view`, `logout_view`, `google_oauth_redirect`, `google_oauth_callback` | Dispatches between dev env-var basic-auth and Teacher (auth.User) login; Google OAuth backs into the same Django ModelBackend so logged-in OAuth users also get `/admin/` access. v1.26.5: the redirect stashes the PKCE `code_verifier` in the session (google-auth-oauthlib ≥1.3 enables PKCE by default) and the callback `pop()`s it onto its fresh Flow — without it every token exchange fails with `invalid_grant: Missing code verifier` |
...
Routes for: login/logout, Google OAuth, password reset (request → confirm → complete), dashboard, schedule, todos, history, support, the `/testing/` QA dashboard, the `/testing/features/` Desarrollos board and its API (v1.21.0), error test pages.
...
- **`QAErrorEmailMiddleware`** (`middleware.py`) — in the QA environment, catches unhandled exceptions and emails them to `SUPPORT_EMAIL` with the full traceback. Toggleable via the `/testing/` dashboard. **v1.23.0:** it is now gated on `IS_TESTING_ENV` and `_redact_body` strips credential fields from the 500-char body preview. The flag lives in the database and the middleware previously honoured it *anywhere* it was set — including production, where the dashboard that toggles it is unreachable but the row is still writable from a shell — so an exception on the login POST emailed `username=…&password=…` in cleartext. Redaction covers **both** urlencoded and multipart bodies; urlencoded alone would cover the login form and silently miss every multipart POST.
...
- **`rate_limit(scope, ...)`** (`rate_limit.py`) — throttles login, 2FA, password reset and
  the parent-portal magic link per client IP. **v1.26.0:** counts with `limit` one-shot
...
- **`set_ready_for_prod on|off`** (v1.26.4) — sets or clears `QAConfiguration.ready_for_prod`, the QA sign-off flag `deploy-production.yml`'s preflight reads through `/health/?deep=1` on testing. The nightly testing deploy runs `off` after every fresh deploy so each version starts locked; `on` exists for manual repair when the `/testing/` button is unreachable — and since v1.26.7 a manual `on` also fires `notify_github_qa_signoff()` (it is still a sign-off, so it arms `Deploy production` the same way the button does; `off` never dispatches anything).
