...
- **`set_ready_for_prod on|off`** (v1.26.4) — sets or clears `QAConfiguration.ready_for_prod`, the QA sign-off flag `deploy-production.yml`'s preflight reads through `/health/?deep=1` on testing. The nightly testing deploy runs `off` after every fresh deploy so each version starts locked; `on` exists for manual repair when the `/testing/` button is unreachable.
