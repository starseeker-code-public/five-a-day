...
- `send_email(template_name, recipients, subject, context, ..., connection=None, reply_to=None, fail_silently=...)` — renders a Django template and sends via SMTP. **v1.30.4 adds `reply_to`**, default `None`, which leaves every existing caller byte-identical (no header, Reply goes to `from_email`). It exists for the portfolio contact relay (`core/views/portfolio.py`), where the person who wrote the message is NOT the SMTP sender: the account belongs to the academy so `From:` cannot change, and without the header, hitting Reply answers the academy's own Gmail rather than the enquirer
...
- `filter_allowed_recipients(addresses)` — **v1.31.0**, the recipient allowlist. Drops every address `settings.EMAIL_ALLOWED_RECIPIENTS` does not permit; an entry is a full address or an `@domain` suffix, matched **case-insensitively** (these are typed by hand into `.env` files and by admins into the app, and `QA@Example.com` failing to match an allowlisted `qa@example.com` would look exactly like the mail silently vanishing). `cc` and `bcc` go through the same filter. The "allowlist is empty outside production" warning is said once per environment per process, tracked in a **set that is only mutated, never rebound** (v1.31.3), so the function needs no `global` statement — which is what CodeQL's `py/unused-global-variable` was reading as a write nothing consumes. It also stops the first environment a process sees from swallowing the warning for every other one, which the test suite does hit: it flips `settings.ENVIRONMENT` between cases.
- `email_service` — singleton instance used throughout the project

> **The allowlist is applied inside `send_email`, and that placement is the whole point** (v1.31.0). Every send funnels through this one method — `send_bulk_emails` delegates to it, as does every convenience function and every Celery task — so it is the only layer that can contain the sends worth demonstrating on the public tester account, which are **transactional and dispatched to a worker**: `send_welcome_email_task` from student creation, the receipt from completing a payment, and the Fun Friday row Beat drains at 14:30, hours after the request ended. The view-level guards in `core/views/app_forms.py` cover the ten mail forms at their two chokepoints and cannot reach any of those.
>
> Two decisions to preserve. **A fully suppressed message returns `True`** — the caller's question is “did this fail?” and the answer is no, the message was handled exactly as configured; returning False would make `_mass_send` count it as an error and show the operator a failure banner for a policy that is working, which is how a containment control gets switched off by somebody debugging a phantom outage. **An empty allowlist means send to everyone**, the opposite of `PORTFOLIO_CONTACT_TOKEN`, because the failure directions are opposite: an unset token defaulting to open makes a public endpoint a mail relay, while an unset allowlist defaulting to closed would silently stop the academy's real payment reminders and receipts in **production**, where this variable is never set. So the containment depends on the VM setting it, and `EmailService` logs one WARNING per process when a non-production environment has left it empty.
...
Templates live in `core/templates/emails/` and extend `emails/base_email.html`. There are currently **22 content templates** (23 files including the shared base): `happy_birthday`, `welcome_student`, `enrollment_child`, `enrollment_adult`, `fun_friday`, `payment_reminder` (a shell with four overridable blocks since v1.29.3), `payment_reminder_september` / `payment_reminder_june` / `payment_reminder_april` (v1.29.3 — the special-month variants, each extending `payment_reminder`), `payment_reminder_simple` (v1.29.5 — an `is_overdue` branch renders a red "vencido … hace N días" box instead of the amber "con vencimiento el …" one, because the weekly window now reaches back over already-overdue rows and a due date read six weeks later is true and useless), `payment_receipt`, `receipt_quarterly_child`, `receipt_adult`, `receipt_enrollment`, `vacation_closure`, `tax_certificate`, `monthly_report`, `admin_monthly_report`, `newsletter`, `parent_temporary_password`, `teacher_activation`, `password_reset`, plus the shared `base_email`.
...
| `send_payment_reminder_email` | `payment_reminder` (or, via `template_name=`, `payment_reminder_september` / `_june` / `_april`) | Monthly manual — takes the fee figures (`full_time_fee`, `part_time_fee`, `part_time_child_fee` — v1.29.4, the media-jornada-infantil band, printed as a sub-line of the part-time row rather than a row of its own — `adult_fee`, `quarterly_fee`, `sibling_full_time_fee`). Any left `None` are filled from `PricingService.payment_reminder_fees()`, so no caller can send the tariff table blank (v1.20.0). `reduced_price_cheque_idioma` carries **no unit** — the template prints `{{ ... }} euros` (see `cheque_idioma_fee`). v1.29.3 adds `template_name`, `subject_suffix` and `extra_context` — the three values `PricingService.payment_reminder_special()` returns for a special month, so the view never picks a template by hand. The September and June variants render **no Cheque Idioma box** (v1.29.4) — the cheque is not applied in the first (half) or last month of the course |
...
| `send_payment_reminders` | Weekly payment reminder batch, deduped against the SMS channel. v1.27.1 reads `timezone.localdate()` **once**: it called `date.today()` twice, so a run straddling midnight built a window whose lower bound was the day *after* its upper bound's reference and silently reminded nobody | Celery Beat (Mondays 09:00) / `send_payment_reminders` command. **v1.29.5**: chases OVERDUE rows (the `due_date__gte=today` lower bound is gone, so `payment_status="pending"` is the only exit condition), reaches ADULT students via `student.email` when there is no parent, and uses a HALF-OPEN `due_date__lt=today+7` upper bound — two consecutive weekly runs of the inclusive window overlapped on one day and double-reminded anything due on a Monday |
