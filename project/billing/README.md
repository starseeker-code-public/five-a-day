...
- **Payment.assert_transition() — the status-transition table, enforced in `save()`** (v1.29.2) — `completed` never from a `DEAD_STATUSES` row (`assert_completable`); `failed` / `cancelled` never from collected or refunded money (`assert_voidable`); `refunded` only from `completed`; `pending` from anywhere. `save()` applies it to every existing row whose `payment_status` is written (skipped when `update_fields` omits the status), so the rule is no longer opt-in: `deactivate_payment`, `update_payment` and the admin used to void or refund rows with a plain `save()` that never ran `clean()`. Views pre-check with the loaded status to answer a friendly 400; `_bulk_set_status` calls `assert_voidable`. Companions: **`is_open`** (`OPEN_STATUSES` = pending, failed) is the one "still collectable / payable" predicate behind the list buttons, the portal's "Pagar online" and `create_checkout_link`; **`is_deletable` / `assert_deletable()`** refuses to hard-delete a completed, refunded or receipt-numbered row, and is read by `delete_payment` and by `PaymentAdmin.has_delete_permission` / `delete_queryset`. Since **v1.29.7** that guard has one deliberate way past it — `purge_with_history`, an explicitly named superuser action behind a confirmation page (`core.admin_purge.PurgeWithHistoryMixin`, also on `EnrollmentAdmin`, where it clears the enrollment's payments first). The guard stays on the ordinary Delete button: it was there to stop an accidental click, not a named decision, and until v1.29.7 the only route to a legitimate deletion was a database shell. The correction path is "Marcar como pendientes", which has to pass `unique_pending_periodic_payment_per_month`
...
The app is at **17 migrations**, through `0017_drop_dead_pricing_columns` (v1.29.5 — **destructive**:
it drops `old_student_discount`, `full_year_bonus`, `half_month_discount`, `one_week_discount` and
`three_week_discount` from `SiteConfiguration`. All five were seeded in `0001_initial` and read by no
service, view, task, template, form, admin or JS file, and `update_site_config` had already been made
to refuse writing them, because `old_student_discount` is visually the twin of the **live**
`returning_student_enrollment_discount` — a price sitting in the database looking authoritative and
applying to nothing. Because it is a `RemoveField`, the production deploy's pre-mutation gate refuses
the release without the `ack_destructive` dispatch input; use that, **not** `force`, which would also
skip the QA sign-off, the provenance gate and the version compare),
`0016_siteconfiguration_part_time_child_monthly_fee_and_more`
(v1.29.4 — `SiteConfiguration.part_time_child_monthly_fee`, seed 32 €, plus the `AlterField` adding
`part_time_child` to `Enrollment.schedule_type`'s choices) and
`0015_alter_payment_unique_receipt_number` (v1.29.3 — the
`AlterConstraint` carrying the Spanish `violation_error_message` that `unique_receipt_number` gained in
v1.29.x without its migration; metadata only, but `makemigrations --check` was red) and
`0014_payment_receipt_number_and_more` (v1.28.2 — `Payment.receipt_number` + the constraint, plus the
`receipt_offset` seed). `0013_siteconfiguration_academy_fields` (v1.27.1 — the five
...
- **Payment.receipt_number / assign_receipt_number()** (v1.28.2) — a stable `YYYY-NNN` receipt number, blank until the first receipt is issued for the payment, then frozen (a family must always see the same number). The counter restarts at 001 each January by the payment's own year (`payment_date` → `due_date` → today); `SiteConfiguration.receipt_offset_year` (2026) instead continues the academy's paper sequence after `receipt_offset` (632), so the first 2026 app receipt is `2026-633`. Assignment is serialised on the singleton `SiteConfiguration` row with `select_for_update`, backed by a partial `unique_receipt_number` constraint (which carries an explicit Spanish `violation_error_message` since v1.29.1 — `full_clean()` validates it, and Django's default renders the constraint NAME into the page). `pdf_service.generate_payment_receipt` calls it; never number a receipt by `payment.id`. **v1.29.1 — it returns `""` for a payment that is not `completed`, without consuming a number**: a recibo is proof that money was collected and the sequence continues the academy's paper books, so issuing one for a pending charge is both a false document and a permanent hole in a fiscal sequence when that charge is later cancelled. Rendering is a GET on an id (`/payments/<id>/receipt.pdf` and the portal's equivalent, which authorises by owner rather than by status), so without the guard a hand-edited URL burned a number on any row the caller could see. The highest number of the year is also read as ONE ordered row rather than pulled into Python inside the config lock — which had made each issue serialise the whole year's history behind itself, and the first full `backfill_drive_receipts` run quadratic.
- **Payment.receipt_date** (v1.29.1) — `payment_date` → `due_date` → today, as one property, because two things key off it and must agree: the YEAR in the `YYYY-NNN` receipt number, and the `Curso`/`<Mes>` folder the Drive archive files the PDF under. Two copies of that fallback chain meant a receipt numbered `2026-6xx` could be filed under a different year's folder, with nothing erroring.
...
- `_get_base_monthly_fee(enrollment, config)` — shared helper that resolves base fee by schedule type (adult_group / full_time / part_time / part_time_child)
...
- `price_breakdown(enrollment, config, months, effective, quarterly) -> (lines, total)` (v1.29.1) — **THE pricing arithmetic.** Returns the itemised lines alongside the total, so the receipt PDF can show a family *why* they paid this figure without re-deriving it. `lines` is `[(label, amount), …]`: the first entry is the full-period base and every later one an amount SUBTRACTED from it, rounded as differences between rounded running subtotals so `base − sum(discounts)` equals `total` to the cent (no residual line papering over half-cent drift). Empty for a hand-priced `special` — a negotiated figure has no standard base to break down against. It exists because the receipt had briefly grown its own copy of the discount order and drifted immediately: x3 cheque idioma on the one-month June stub, sibling/cheque gated on `student.is_adult` instead of `schedule_type`, and a phantom `Ajuste +0,01 €` from per-line rounding. **v1.29.3 changes the ORDER: percentages → the flat cheque idioma × months in the period ("Cheque idioma" / "Cheque idioma (3 meses)") → proration of the NET figure → June.** The cheque used to be scaled by `effective`, so a prorated September receipt printed "Cheque idioma −10,67 €" for a family holding a 20 € cheque; the total is identical either way (`(base − 20) × f == base × f − 20 × f`, pinned by `test_qa_pickup_receipts_and_reminders.py`), only the lines changed.
- `_price_months(enrollment, config, months, effective, quarterly)` (v1.27.1) — the discount ORDER, split out of `calculate_period_amount` so a period short at **both** ends can be priced by the same code; since v1.29.1 it is the total-only face of `price_breakdown`. `calculate_period_amount` expresses "the first month is partial" (one fraction, on month 0), which is all the schedule generator needs; `close_out_periods` bills a block truncated by a plan change and hands in the effective total directly. Any new amount calculation must come through it — a second copy of that order is a family charged one figure and quoted another.
- `_get_base_monthly_fee(enrollment, config)` — delegates to `billing.money.monthly_fee_for` (v1.29.1). It used to be a third hand-rolled copy of the schedule → fee mapping beside `PricingService.get_monthly_fee`, so a schedule type added to one map priced correctly on the ficha and fell back to full-time on the invoice.
...
- `get_monthly_fee(schedule_type)` — fee by full_time/part_time/part_time_child/adult_group. Since v1.29.1 a thin delegate to `billing.money.monthly_fee_for`, which `PaymentService._get_base_monthly_fee` also calls — one mapping, so the ficha and the invoice cannot price a new schedule type differently
...
- `payment_reminder_fees(config=None)` (v1.20.0) — the display-ready fee table for the `payment_reminder` email (full-time, part-time, adult, quarterly, sibling full-time, plus `part_time_child_fee` since v1.29.4 — which rides WITH the part-time row as a sub-line rather than becoming a sixth row: it is the same class at another price), formatted the way the Spanish emails print money (comma decimal, no `,00` tail). The quarterly and sibling rows used to read *"consultar en la academia"*; every caller that renders the template now shares one source of truth
- `payment_reminder_special(config=None, month_number=None, *, september_start_day=None)` (v1.29.3) — everything the reminder email needs for a month: `special_case` (`None` / `"september"` / `"june"` / `"april"`, from the module-level `REMINDER_SPECIAL_MONTHS`), `template_name` (`REMINDER_TEMPLATES`), `subject_suffix`, the tariff rows (incl. `part_time_child_fee` since v1.29.4) and `reduced_price_cheque_idioma` ALREADY ADJUSTED for the month, every regular figure under `standard_*`, and `september_start_day` / `proration_percent` / `june_discount`. September prorates each monthly row from the start day (default `SEPTEMBER_CLASSES_START_DAY` — **16** since v1.29.4, validated 1-30 by `_september_start_day`) with `PaymentService.proration_fraction`; June takes `june_discount` off every monthly row (adults excluded); April discounts the quarterly row (the Apr-Jun block contains June). Every figure comes from `_standard_period_price`. **September and June render no Cheque Idioma box at all** (v1.29.4): the cheque is a fixed amount per month and the academy applies it in neither the first (half) month nor the last of the course, so quoting a figure there named a price nobody is charged — both templates override that block empty.
- `_standard_period_price(config, *, months, schedule_type="full_time", quarterly=False, fraction=1, sibling=False, cheque=False)` (v1.29.3) — prices a STANDARD period through `PaymentService.calculate_period_amount` on a bare `SimpleNamespace` carrier (the generator reads only `schedule_type`, `is_sibling_discount`, `has_language_cheque`, `is_hand_priced`, `final_amount`), so an advertised figure is by construction the billed one. Lazy import — `payment_service` imports `billing.models`, which imports this module.
- `cheque_idioma_price(config=None)` (v1.29.3) — the full-time monthly fee with the Cheque Idioma, as a `Decimal`, priced through `_standard_period_price`; `comms.services.email_functions.cheque_idioma_fee` formats it. It replaces a `fee − discount` subtraction in `comms`.
- `reminder_case_for_month(month_number)` (v1.29.3) — `REMINDER_SPECIAL_MONTHS.get(...)`.
...
- `qa_card_amounts(today)` — the `/app/testing/` "Gastos GCP" line: previous month (archived row
...
- `generate_payment_receipt(payment)` — single-payment receipt PDF (v1.3, reportlab). v1.28.2: assigns the payment its `YYYY-NNN` receipt number on first issue and prints it. **v1.29.1 — an uncollected payment gets no number**: `assign_receipt_number()` returns blank unless the payment is `completed`, and the document renders headed plain `RECIBO` rather than failing (every caller already filters on `completed`, so this is the belt-and-braces half of that rule)
- `_receipt_breakdown_rows(payment)` — the `precio base − descuentos = importe` table, **reconstruct-or-say-nothing** since v1.29.1. The lines come from `PaymentService.price_breakdown`, the same call that prices the payment, and are shown ONLY when re-pricing the period reproduces `payment.amount` to the cent; anything else (a hand-priced special, a manual correction, a close-out block, or a receipt re-downloaded after prices changed in `/app/management/`) falls back to the bare `Importe` line. It previously re-derived the discounts itself and printed whatever was left over as "Prorrateo primer periodo" / "Ajuste", so any disagreement with the generator was silently relabelled as proration on a document families keep for tax purposes — a one-month June stub showed a full quarter's base and a ~2/3 "prorrateo", and re-downloading last September's receipt after a price rise restated its base at the NEW price
- `_matricula_breakdown_rows(enrollment, student, config, amount)` (v1.29.1) — the matrícula equivalent. Only two figures are explainable from the configuration (the standard fee, and that fee minus the returning-student discount), and the discount line is gated on the enrollment's own **category**, not on the arithmetic: deriving it from `base − amount` labelled ANY shortfall "Descuento antiguo alumno", so a negotiated 25 € matrícula against a 40 € standard fee printed a 15 € returning-student discount for a family that had never studied here
...
types into `/app/management/` and they land in a `Paragraph`, so an academy name containing `&` or `<`
...
- **EnrollmentForm** — the enrollment half of the student-creation page. Plain `forms.Form` (not a
  ...
  - `enrollment_plan` — `monthly_full` / `monthly_part` / `monthly_part_child` (v1.29.4, the media-jornada-infantil band: the `monthly_part` timetable at its own cheaper rate) / `quarterly`
  - `has_language_cheque`, `is_sibling_discount`, `sibling_id`, `is_special`
  ...
  - `manual_amount` — **Precio manual (€)**, the *recurring* fee (per month, or per quarter on a
    quarterly plan). Takes effect only when `customize_recurring` ("Personalizar también la cuota")
    is ticked — `clean()` clears it otherwise, so a stale value cannot sneak a hand price on.
    **Since v1.29.1 `__init__` owns the converse invariant**: a pre-filled `manual_amount` implies
    that box, and ticks it. `clean()` discards the amount whenever the box is off and then rejects
    the special, so a caller that pre-filled a hand price without also ticking the companion flag
    turned every save of that student's ficha — even a phone-number edit — into a silent no-op, with
    the form's non-field errors the only signal. Owning it in the form means the next surface that
    edits a special enrollment cannot reintroduce that bug by forgetting the flag.
...
- Choice tuples: ENROLLMENT_TYPE_CHOICES, SCHEDULE_TYPE_CHOICES (four bands since v1.29.4 — `full_time`, `part_time`, `part_time_child` "Infantil (1 día/semana)", `adult_group`), PAYMENT_MODALITY_CHOICES, etc.
...
- `RECEIPT_RELATIONS` (v1.29.9) — the `select_related` set everything `generate_payment_receipt` touches off a `Payment`: the receipt's discount breakdown reads the enrollment and its matrícula category, and `enrollment.student` is a separate FK cache from `payment.student`. It lives **here**, in billing, because it describes `Payment`'s own relations and because BOTH receipt tasks need it — the email one in `comms.tasks`, the Drive one in `core.tasks` — and those two may not import each other, while both may import billing. Without it each send paid 3 extra lazy queries per payment, which production (eager Celery) spends inside the completion request and the Drive backfill multiplies by the whole archive
...
One-off (v1.29.0) that uploads receipts for payments **already completed** before the Drive archive existed — the on-completion upload only covers new ones. Idempotent: the service skips any payment whose receipt is already in its month folder, so it is safe to re-run. Best-effort per payment (one render/upload failure is reported and the run continues). Since v1.29.1 an unconfigured Drive raises `CommandError` rather than printing a message and returning 0 — it runs as a Cloud Run Job, where a clean exit over an empty archive reported success for a run that did nothing — and the dry run answers from the count instead of streaming the whole joined archive to re-count it. **v1.29.5 — it also refuses outside production**, up front rather than once per payment: it builds its own `DriveReceiptService`, so without the check a run on the QA VM would write real archive paths, and a refusal reported per row reads like a Drive outage rather than a switched-off feature. As of **v1.29.10** that refusal is absolute — the QA opt-in and its `testing/` sandbox are gone, because the uploader is now a delegated Google account and the QA VM cannot complete that consent at all. It also needs an account **connected** from `/app/management/`, not just the folder id. See `core/services/drive_service.py`.
