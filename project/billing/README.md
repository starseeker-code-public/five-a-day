...
The app is at **15 migrations**, through `0015_alter_payment_unique_receipt_number` (v1.29.3 — the
...
- `get_monthly_fee(schedule_type)` — fee by full_time/part_time/adult_group. Since v1.29.1 a thin delegate to `billing.money.monthly_fee_for`, which `PaymentService._get_base_monthly_fee` also calls — one mapping, so the ficha and the invoice cannot price a new schedule type differently
